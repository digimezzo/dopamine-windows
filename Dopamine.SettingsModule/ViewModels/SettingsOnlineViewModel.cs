    ...
    public class SettingsOnlineViewModel : BindableBase
    {
        ...
        private async void GetCheckBoxesAsync()
        {
            await Task.Run(() =>
            {
                ...
                OnPropertyChanged(() => this.CheckBoxNeteaseLyricsChecked);
            });
        }
        ...
        private void AddRemoveLyricsDownloadProvider(string provider, bool add)
        {
            ...
            {
                CoreLogger.Error("Could not add/remove lyrics download providers. Add = '{0}'. Exception: {1}", add.ToString(), ex.Message);
            }
        }
