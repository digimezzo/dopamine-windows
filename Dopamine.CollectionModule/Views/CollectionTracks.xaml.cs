    ...
    public partial class CollectionTracks : CommonTracksView, INavigationAware
    {
        ...
        private void ViewInExplorer(DataGrid iDataGrid)
        {
            if (iDataGrid.SelectedItem != null)
            {
                try
                {
                    Actions.TryViewInExplorer(((MergedTrackViewModel)iDataGrid.SelectedItem).MergedTrack.Path);
                }
                catch (Exception ex)
                {
        ...
        protected async Task TracksKeyUpHandlerAsync(object sender, KeyEventArgs e)
        {
            ...
            else if (e.Key == Key.E && Keyboard.Modifiers == ModifierKeys.Control)
            {
                ...
                {
                    Actions.TryViewInExplorer(((MergedTrackViewModel)dg.SelectedItem).MergedTrack.Path);
                }
            }
        ...
        {
            this.Subscribe();
            XmlSettingsClient.Instance.Set<int>("FullPlayer", "SelectedPage", (int)SelectedPage.Tracks);
        }
        #endregion
    }
}
