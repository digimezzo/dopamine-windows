    ...
    public class ProviderService : IProviderService
    {
        ...
        private void LoadProvidersXml()
        {
            if (this.providersDocument == null)
            {
                ...
                {
                    LogClient.Current.Error("Could not load providers XML. Exception: {0}", ex.Message);
                }
        ...
        public async Task<List<SearchProvider>> GetSearchProvidersAsync()
        {
            ...
            await Task.Run(() =>
            {
                ...
                {
                    LogClient.Current.Error("Could not load search providers. Exception: {0}", ex.Message);
                }
        ...
        public void SearchOnline(string id, string[] searchArguments)
        {
            ...
            {
                LogClient.Current.Error("Could not search online using url: '{0}'. Exception: {1}", url, ex.Message);
            }
        }
        ...
        public bool RemoveSearchProvider(SearchProvider provider)
        {
            ...
            if (providerElementToRemove != null)
            {
                ...
                {
                    LogClient.Current.Error("Could not remove search provider. Exception: {0}", ex.Message);
                }
            }
        ...
        public UpdateSearchProviderResult AddSearchProvider(SearchProvider provider)
        {
            ...
            {
                LogClient.Current.Error("The online search provider could not be added. Fields 'Name' and 'Url' are required, 'Separator' is optional.");
            ...
            {
                LogClient.Current.Error("Could not update search provider. Exception: {0}", ex.Message);
            }
        ...
        public UpdateSearchProviderResult UpdateSearchProvider(SearchProvider provider)
        {
            if (string.IsNullOrEmpty(provider.Name) | string.IsNullOrEmpty(provider.Url))
            {
                LogClient.Current.Error("The online search provider could not be updated. Fields 'Name' and 'Url' are required, 'Separator' is optional.");
                return UpdateSearchProviderResult.MissingFields;
            }
