        ...
        public async Task SuspendAsync()
        {
            ...
            try
            {
                await Task.Run(() =>
                {
                    ...
                    if (stepsRequired > 0)
                    {
                        int stepDuration = Convert.ToInt32(250 / stepsRequired);
            ...
            catch (Exception ex)
            {
                ...
                LogClient.Instance.Logger.Error("An error occurred during Fade out. Exception: {0}", ex.Message);
            }
        ...
        public async Task UnsuspendAsync()
        {
            ...
            try
            {
                await Task.Run(() =>
                {
                    ...
                    int stepsRequired = Convert.ToInt32(this.volume / 0.05);

                    if (stepsRequired > 0)
                    {
                        int stepDuration = Convert.ToInt32(250 / stepsRequired);

                        while (currentVolume < this.volume)
                        {
                            var volumeToSet = Convert.ToSingle(currentVolume + 0.05 <= this.volume ? currentVolume + 0.05 : this.volume);
                            this.player.SetVolume(volumeToSet);
                            currentVolume = this.player.GetVolume();
                            Thread.Sleep(stepDuration);
                        }
            ...
            catch (Exception ex)
            {
                ...
                LogClient.Instance.Logger.Error("An error occurred during Fade in. Exception: {0}", ex.Message);
            }
        }
