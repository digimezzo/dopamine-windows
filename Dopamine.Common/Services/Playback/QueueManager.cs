...
namespace Dopamine.Common.Services.Playback
{
    internal class QueueManager
    {
        ...
        private bool UpdateTrackPlaybackInfo(PlayableTrack track, FileMetadata fileMetadata)
        {
            ...
            {
                CoreLogger.Error("Could not update the track metadata. Exception: {0}", ex.Message);
            }
        ...
        public KeyValuePair<string, PlayableTrack> CurrentTrack()
        {
            ...
            {
                CoreLogger.Error("Could not get current track. Exception: {0}", ex.Message);
            }
        ...
        public KeyValuePair<string, PlayableTrack> FirstTrack()
        {
            ...
            {
                CoreLogger.Error("Could not get first track. Exception: {0}", ex.Message);
            }
        ...
        public async Task<KeyValuePair<string, PlayableTrack>> PreviousTrackAsync(LoopMode loopMode)
        {
            ...
            await Task.Run(() =>
            {
                ...
                {
                    CoreLogger.Error("Could not get previous track. Exception: {0}", ex.Message);
                }
            });
        ...
        public async Task<KeyValuePair<string, PlayableTrack>> NextTrackAsync(LoopMode loopMode)
        {
            ...
            await Task.Run(() =>
            {
                ...
                {
                    CoreLogger.Error("Could not get next track. Exception: {0}", ex.Message);
                }
            });
        ...
        public async Task<EnqueueResult> EnqueueAsync(IList<PlayableTrack> tracks, bool shuffle)
        {
            ...
            catch (Exception ex)
            {
                ...
                CoreLogger.Error("Error while enqueuing tracks. Exception: {0}", ex.Message);
            }
        ...
        public async Task<EnqueueResult> EnqueueNextAsync(IList<PlayableTrack> tracks)
        {
            ...
            {
                result.IsSuccess = false;
                CoreLogger.Error("Error while enqueuing tracks next. Exception: {0}", ex.Message);
            }
        ...
        public async Task<bool> ClearQueueAsync()
        {
            ...
            await Task.Run(() =>
            {
                ...
                {
                    isSuccess = false;
                    CoreLogger.Error("Error while clearing queue. Exception: {0}", ex.Message);
                }
            });
            ...
            await Task.Run(() =>
            {
                lock (this.queueLock)
                {
                    foreach (var track in tracks)
                    {
                        ...
                        {
                            isSuccess = false;
                            CoreLogger.Error("Error while removing queued track with path='{0}'. Exception: {1}", track.Value.Path, ex.Message);
                        }
                    }
                    ...
                    foreach (var dequeuedTrack in dequeuedTracks)
                    {
                        ...
                        {
                            isSuccess = false;
                            CoreLogger.Error("Error while removing shuffled track with path='{0}'. Exception: {1}", dequeuedTrack.Value.Path, ex.Message);
                        }
                    }
                }
            });
        ...
        public async Task<bool> UpdateQueueOrderAsync(List<KeyValuePair<string, PlayableTrack>> tracks, bool isShuffled)
        {
            ...
            {
                isSuccess = false;
                CoreLogger.Error("Could update queue order. Exception: {0}", ex.Message);
            }
